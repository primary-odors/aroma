...
public:
    ...
    int get_overlap_count(Atom** atoms);
    RING_TYPE get_type();
    Point get_center();
    Vector get_normal();
...
public:
    ...
    Atom* get_heavy_atom();
    bool is_nearest_hydrogen(Atom* to);
...
public:
    const int& Z = m_Z;
    const float& vdW_radius = vdW_rad;
    const Point& loc = location;
};
