...
float (*conj_get_charge)(void* lconjugation) = nullptr;
...
void Atom::read_elements()
{
    ...
    else
    {
        ...
        char* got = getcwd(buffer, PATH_MAX);
...
void Atom::figure_out_valence()
{
    ...
    if (elem_syms[m_Z] && geometries[m_Z])
    {
        valence = valences[m_Z];
        geometry = geometries[m_Z];
        if (valence > abs(geometry)) cout << "ERROR Valence for " << elem_syms[m_Z] << " exceeds geometry! Check values in elements.dat are correct.\n";
    }
    else
    {
    ...
    if (m_Z >=  21 && m_Z <=  30) family = HEAVYMETAL;
    if (m_Z >=  39 && m_Z <=  48) family = HEAVYMETAL;
    if (m_Z >=  57 && m_Z <=  71) family = LANTHANIDE;
    if (m_Z >=  72 && m_Z <=  80) family = HEAVYMETAL;
    if (m_Z >=  89 && m_Z <= 103) family = ACTINIDE;
    if (m_Z >= 104 && m_Z <= 112) family = HEAVYMETAL;

    elecn = electronegativities[m_Z];
    Eaffin = elec_affinities[m_Z];
    at_wt = atomic_weights[m_Z];
    Eion = ioniz_energies[m_Z];
    location.weight = at_wt;
    vdW_rad = vdW_radii[m_Z];
    Sphere s;
    s.radius = vdW_rad;
    molsurf_area = s.area();
}
        ...
        if (words)
        {
               ...
               )
            {
                try
                {
                    ...
                    m_Z = Z_from_esym(esym);
                    if (!m_Z && !strcmp(name, "OXT")) m_Z = 8;
...
Atom::Atom(const Atom &cf)
{
    pK = cf.pK;
    pdbidx = cf.pdbidx;
    residue = cf.residue;
    aaletter = cf.aaletter;
    strcpy(aa3let, cf.aa3let);
    region = new char[cf.region ? strlen(cf.region) : 8];
    if (cf.region) strcpy(region, cf.region);
    is_backbone = cf.is_backbone;
    name = new char[cf.name ? strlen(cf.name) : 8];
    if (cf.name) strcpy(name, cf.name);
    mirror_geo = cf.mirror_geo;
    flip_mirror = cf.flip_mirror;
    dnh = cf.dnh;
    EZ_flip = cf.EZ_flip;
    pdbchain = cf.pdbchain;
    molsurf_area = cf.molsurf_area;
    m_Z = cf.m_Z;
    ...
    charge = cf.charge;
    origchg = cf.origchg;
    acidbase = cf.acidbase;
    polarity = cf.polarity;
    polar_calcd = cf.polar_calcd;
    thiol = cf.thiol;
    family = cf.family;
    swapped_chirality = cf.swapped_chirality;
    chirality_unspecified = cf.chirality_unspecified;
    recursion_counter = cf.recursion_counter;
    is_imidazole_like = cf.is_imidazole_like;
}
...
{
    int i;
    if (!check_moves_with_cache() || !moves_rigidly_with_atom2)
    {
        result[0] = nullptr;
        return;
    }
...
{
    int i;
    if (!check_moves_with_cache()) return true;
...
float Atom::hydrophilicity_rule()
{
    ...
    else if (m_Z == 7 || m_Z == 8) total += 1;
    else if (m_Z == 15) total += 0.666;
    else if (m_Z == 16 && is_bonded_to("H")) total += 0.5;
...
void Atom::fetch_bonds(Bond** result)
{
    ...
    try
    {
        ...
        for (i=0; i<geometry; i++)
        {
            ...
            if (!bonded_to[i].atom1->m_Z) continue;
...
float Atom::get_bond_card_sum()
{
    if (!bonded_to) return 0;
    int i;
    float retval=0;
    for (i=0; i<geometry; i++) if (bonded_to[i].atom2) retval+=bonded_to[i].cardinality;
    return retval;
}

int Atom::get_num_lone_pairs()
{
    if (!bonded_to) return 0;
    int bc = get_bonded_atoms_count();
    if (is_metal()) return max(0, geometry-bc);
    else if (is_pi()) return max(0, 3-bc);
    else return max(0, 4-bc);
}
...
int Atom::get_bonded_heavy_atoms_count()
{
    if (!bonded_to) return 0;
    int i, retval=0;
    for (i=0; i<geometry; i++) if (bonded_to[i].atom2 && bonded_to[i].atom2->m_Z > 1) retval++;
    return retval;
}
...
Atom* Atom::get_heavy_atom()
{
    ...
    int i;
    for (i=0; i<geometry; i++)
        if (bonded_to[i].atom2 && bonded_to[i].atom2->m_Z > 1)
            return bonded_to[i].atom2;
    return this;
}

bool Atom::is_nearest_hydrogen(Atom *to)
{
    if (Z > 1) return false;            // no, because it isn't a hydrogen.
    Atom* heavy = get_heavy_atom();
    if (!heavy || heavy == this) return false;
    int i;
    float r = distance_to(to);
    for (i=0; i<heavy->geometry; i++)
    {
        Atom* a = heavy->bonded_to[i].atom2;
        if (!a) continue;
        if (a == this) continue;
        if (a->Z > 1) continue;
        if (a->distance_to(to) < r) return false;
    }
    return true;
}
...
Bond* Atom::bond_to(Atom* latom2, float lcard)
{
    ...
    // if (this < latom2 && m_Z > 1 && latom2->m_Z > 1) cout << "Bond " << name << cardinality_printable(lcard) << latom2->name << endl;
...
float Atom::is_polar()
{
    if (!polar_calcd)
    {
        #if conj_charge_as_polarity
        if (family != TETREL && conjugation && (Z > 1 || (bonded_to[0].atom2 && bonded_to[0].atom2->family != TETREL)))
        {
            polarity = sgn(is_conjugated_to_charge());
            polar_calcd = true;
            return polarity;
        }
        #endif
...
int Atom::is_thio()
{
    if (!thiol)
    {
        ...
        else if (family == CHALCOGEN && m_Z > 8 && is_bonded_to("H"))
        {
            if (m_Z == 16) thiol = -1;
            else if (m_Z == 34) thiol = -0.5;
            else if (m_Z == 52) thiol = -0.2;
        }
    }
    return thiol;
}
...
bool Atom::can_coord_metals()
{
    if ((family == CHALCOGEN || family == PNICTOGEN) && elecn >= 2.08) return true;
    return false;
}

bool Atom::is_pi()
{
    if (picache >= 0) return (picache>0);
    if (!bonded_to)
    {
        picache = 0;
        return false;
    ...
    if (m_Z == 1)
    {
        if (bonded_to[0].atom2 && bonded_to[0].atom2->m_Z > 1) return bonded_to[0].atom2->is_pi();
    }
    ...
    if (family == CHALCOGEN && is_bonded_to_pi(TETREL, true) && !is_bonded_to(PNICTOGEN))
    {
        picache = 1;
        return true;
    }
...
int Atom::get_geometry()
{
    if (valence==4 && !is_pi()) geometry = valence;
    if ((family == PNICTOGEN || family == CHALCOGEN) && m_Z < 10 && !is_pi()) geometry = 4;
    return geometry;
}
        ...
        if (bonded_to)
        {
            ...
            for (i=0; i<geometry; i++)
            {
                ...
                if (C)
                {
                    ...
                    int m_Z = C->m_Z;
                    if (m_Z != 1 && fam != TETREL) return false;
                    else
                    {
...
bool Bond::check_moves_with_cache()
{
    if (moves_with_atom2 && moves_rigidly_with_atom2)
    {
        if (abs(reinterpret_cast<long>(moves_with_atom2) - reinterpret_cast<long>(moves_rigidly_with_atom2)) > memsanity)
            moves_with_atom2 = moves_rigidly_with_atom2 = nullptr;
        else if (abs(reinterpret_cast<long>(moves_with_atom2) - reinterpret_cast<long>(this)) > memsanity)
            moves_with_atom2 = moves_rigidly_with_atom2 = nullptr;
        else if (moves_with_atom2[0] && abs(reinterpret_cast<long>(moves_with_atom2[0]) - reinterpret_cast<long>(this)) > memsanity)
            moves_with_atom2 = moves_rigidly_with_atom2 = nullptr;
        else if (moves_rigidly_with_atom2[0] && abs(reinterpret_cast<long>(moves_rigidly_with_atom2[0]) - reinterpret_cast<long>(this)) > memsanity)
            moves_with_atom2 = moves_rigidly_with_atom2 = nullptr;
    }
    if (!moves_with_atom2)
    {
    ...
    return moves_with_atom2 != nullptr;
}
...
{
    return atom2->loc.subtract(atom1->loc);
}
...
_cannot_reverse_bondrot:
    ...
    #if limit_flexions_by_mclash
    float old = (atom2->residue && atom2->mol && preflex_cb) ? (preflex_cb)(atom2->mol) : 0;
    #endif
    ...
    #if limit_flexions_by_mclash
    bool accepted = (atom2->residue && atom2->mol && postflex_cb) ? (postflex_cb)(atom2->mol, old) : true;
...
Point Bond::ring_rotate(float theta, Atom* sa)
{
    ...
    check_moves_with_cache();

    int i, j, n;
    Atom* moves_with_sa[256];
    memset(moves_with_sa, 0, 256*sizeof(Atom*));
    if (sa)
    {
        Bond* bstop = nullptr;
        n = sa->get_bonded_atoms_count();
        j = 0;
        for (i=0; i<n; i++)
        {
            bstop = sa->get_bond_by_idx(i);
            if (!bstop->atom2) continue;
            if (bstop->atom2->in_same_ring_as(sa)) continue;
            if (bstop)
            {
                moves_with_sa[j++] = bstop->atom2;
                bstop->fetch_moves_with_atom2(&moves_with_sa[j]);
                for (j=0; moves_with_sa[j]; j++);
            }
        }
    }
    ...
    // for (j=0; moves_with_sa[j]; j++) cout << moves_with_sa[j]->name << " "; cout << endl;
    ...
    for (i=0; moves_with_atom2[i]; i++)
    {
        ...
        a2isbsa = false;
        if (moves_with_atom2[i] == sa) a2isbsa = true;
        else for (j=0; moves_with_sa[j]; j++) if (moves_with_atom2[i] == moves_with_sa[j]) a2isbsa = true;
...
float Ring::flip_atom(Atom* wa)
{
    ...
    Vector auv = va->loc.subtract(ua->loc);
    Vector avx = xa->loc.subtract(va->loc);
    Vector ayx = xa->loc.subtract(ya->loc);

    // cout << "Atoms:" << endl << "U: " << ua->name << endl << "V: " << va->name << endl << "W: " << wa->name << endl << "X: " << xa->name << endl << "Y: " << ya->name << endl;

    float theta = 0, step = 0.1*fiftyseventh, bestr = Avogadro, bthet = 0;
    Point ol = wa->loc, pt1, pt2, nl;
    ...
    while (theta<M_PI*2)
    {
        ...
        pt1 = rotate3D(ol, va->loc, auv,  theta);
        pt2 = rotate3D(ol, xa->loc, ayx, -theta);
        ...
        if (theta >= 20*fiftyseventh && theta <= 340*fiftyseventh)
        {
            r = pt1.get_3d_distance(pt2);
            // cout << (theta*fiftyseven) << "deg: r = " << r << " step = " << (step*fiftyseven) << endl << flush;
    ...
    int i, j;
    Point origin = xa->loc;
    float vxtheta = -find_angle_along_vector(wa->loc, ol, origin, avx);
    if (1) for (i=0; wbonds[i]; i++)
    {
        if (!wbonds[i]->atom2) continue;
        if (wbonds[i]->atom2->in_same_ring_as(wa)) continue;

        Point lpt = wbonds[i]->atom2->loc;
        lpt = rotate3D(lpt, origin, avx, vxtheta);
...
bool Bond::is_equivalent(Bond* cmp_to)
{
    if (!moves_with_atom2) fill_moves_with_cache();
    if (!cmp_to->moves_with_atom2) cmp_to->fill_moves_with_cache();

    #if _dbg_chirality_detection
    cout << "Bond: " << atom1->name << "-" << atom2->name
        << " compared to " << cmp_to->atom1->name << "-" << cmp_to->atom2->name
        << ":" << endl;
    cout << "Atom counts: " << mw_atom_count << " / " << cmp_to->mw_atom_count << endl;
    cout << "Z sum: " << mw_Zsum << " / " << cmp_to->mw_Zsum << endl;
    cout << "Card sum: " << mw_cardsum << " / " << cmp_to->mw_cardsum << endl;
    cout << endl;
    #endif
...
{
    if (!check_moves_with_cache()) return;
    ...
    for (i=0; moves_with_atom2[i]; i++)
    {
        Point ol = moves_with_atom2[i]->loc;
...
Vector* Atom::get_geometry_aligned_to_bonds(bool prevent_infinite_loop)
{
    ...
    if (origgeo == 2 && bc == 1)
    {
        if (!geov) geov = new Vector[origgeo];
        geov[0] = bonded_to[0].get_axis();
        geov[1] = Point(0,0,0).subtract(geov[0]);
    ...
    {
        Vector B0 = bonded_to[0].atom2->location.subtract(this->location);
        ...
        if (geometry>1)
        for (j=1; j<geometry; j++) if (bonded_to[j].atom2)
        {
...
{
    if (!check_moves_with_cache()) return 0;
    int i;
...
{
    if (!check_moves_with_cache() || !moves_rigidly_with_atom2) return 0;
    int i;
...
float Atom::is_conjugated_to_charge(Atom* bir, Atom* c)
{
    ...
    {
        Atom* heavy = get_heavy_atom();
        if (heavy && heavy->Z > 1) return heavy->is_conjugated_to_charge(this, this);
    }
    if (!is_pi())
    {
        return charge;
    }
    if (conjugation && conj_get_charge) return conj_get_charge(reinterpret_cast<void*>(conjugation));
    else return 0;
...
{
    if (a->m_Z < 2) return (is_conjugated_to(a->bonded_to[0].atom2));
...
Atom** Ring::get_atoms() const
{
    ...
    for (i=0; i<atcount; i++)
    {
        j = atno_from_aname(atoms[i]->name);
        if (!i || j < first)
        {
            first = j;
            i1 = i;
    ...
    Atom* prev = atoms[(i1+atcount-1)%atcount];
    Atom* next = atoms[(i1+1)%atcount];

    dir = sgn(atno_from_aname(prev->name) - atno_from_aname(next->name));
    if (!dir) dir = sgn(prev - next);

    j=0;
    for (i=i1; j<atcount; i+=dir)
    {
        if (i>=atcount) i -= atcount;
        else if (i<0) i += atcount;
        retval[j++] = atoms[i];
    }
...
int Ring::get_overlap_count(Atom** a)
{
    int i, j, m = get_atom_count(), n, result = 0;
    for (n=0; a[n]; n++);
    for (i=0; i<m; i++)
    {
        for (j=0; j<n; j++)
        {
            if (atoms[i] == a[j]) result++;
        }
    }
    return result;
}
...
bool Ring::is_coplanar()
{
    ...
    else
    {
        ...
        for (i=3; i<atcount; i++)
        {
            anomaly = are_points_planar(atoms[0]->loc,
                                        atoms[1]->loc,
                                        atoms[2]->loc,
                                        atoms[i]->loc
                                       );
