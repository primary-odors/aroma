    ...
    BestBindingResult* output, Cavity* container, bool allow_thiolation, Progressbar *pbr)
{
    int i, j, k, l, m, n, ii;
    float best = 0;
    bool require_ionic = false, require_neutral_polar = false, require_pi = false, require_metal = false, require_amine_aldehyde = false;
    ...
    for (i=0; i<ntarg; i++)
    {
        ...
        for (j=0; pocketres[j]; j++)
        {
            ...
            bool jmet = pocketres[j]->coordmtl;
            if (jpol < hydrophilicity_cutoff && !jhis) jpol = 0;

            if (jmet) require_metal = true;
            else if (is_ald && pocketres[j]->is_amine()) require_amine_aldehyde = true;
            else if (ichg && jchg && sgn(ichg) == -sgn(jchg)) require_ionic = true;
            else if (ipol && jpol) require_neutral_polar = true;
        }
    }
    ...
    for (i=0; i<ntarg; i++)
    {
        ...
        if (require_metal && !targets[i].can_coord_metals()) continue;
        ...
        for (j=0; pocketres[j]; j++)
        {
            ...
            if (require_metal && !pocketres[j]->coordmtl) continue;
            ...
            #endif
            {
                ...
                if (!require_amine_aldehyde && !require_metal && kimp > iimp) continue;
                ...
                for (l=0; pocketres[l]; l++)
                {
                    ...
                    if (!pocketres[l]->coordmtl && !require_amine_aldehyde && !require_metal)
                    ...
                    if (container) r = r / container->get_center().get_distance_to_line(pocketres[l]->get_CA_location(), pocketres[j]->get_CA_location());
                    ...
                    if (shadowed && !require_amine_aldehyde && !require_metal) continue;
                }
                ...
                for (l=0; pocketres[l]; l++)
                {
                    ...
                    for (m=-1; m<ntarg; m++)
                    {
                        ...
                        if (mimp > iimp && !require_amine_aldehyde && !require_metal) continue;

                        if (m >= 0)
                        {
                            if (!require_amine_aldehyde && !require_metal && m<=k) continue;
                        ...
                        {
                            if (m >= 0 && !require_amine_aldehyde && !require_metal && ntarg>=3 && (n==j || n==l)) continue;
...
bool LigandTarget::is_aldehyde()
{
    ...
    if (O)
    {
        Atom *C = O->is_bonded_to(TETREL, 2);
        if (C)
        {
...
bool LigandTarget::can_coord_metals()
{
    if (single_atom) return single_atom->can_coord_metals();
    Atom *a = contains(CHALCOGEN);
    if (!a) a = contains(PNICTOGEN);
    if (a) return a->can_coord_metals();
    return false;
}
