...
void AminoAcid::find_his_flips()
{
    ...
    for (i=0; atoms[i]; i++)
    {
        ...
        if (atoms[i]->is_pi() && atoms[i]->get_family() == TETREL)
        {
            Atom* a = atoms[i]->is_bonded_to(PNICTOGEN);
            if (a && !a->get_charge())
            {
                Atom *Nfrom = nullptr, *Nto = nullptr, *H = nullptr;

                int j, g = atoms[i]->get_geometry();
                for (j=0; j<g; j++)
                {
                    ...
                    a = b->atom2;
                    if (!a) continue;
                    if (a->get_charge()) continue;
                    if (!a->is_pi()) continue;
                    if (a->get_family() == PNICTOGEN)
                    {
                        Atom* ah = a->is_bonded_to("H");
                        if (ah && Nfrom)
                        {
                            Nfrom = H = nullptr;
                            break;
        ...
        for (i=0; atoms[i]; i++)
        {
            ...
            for (j=0; j<bg; j++)
            {
                ...
                if (bb[j]->atom2 && bb[j]->atom2 < bb[j]->atom1)
                {
                    ...
                    aabd[n]->Za = bb[j]->atom1->Z;
                    aabd[n]->Zb = bb[j]->atom2->Z;
...
void AminoAcid::predict_previous_COCA(Point* retval)
{
    ...
    for (i=0; i<10; i++)
    {
        ...
        Point maybeP = rotate3D(prevCloc, N->loc, axis, plus);
        Point maybeM = rotate3D(prevCloc, N->loc, axis, minus);
        lv.copy(axis);
        lv.origin = N->loc;
        if (maybeP.get_3d_distance(CA->loc) > maybeM.get_3d_distance(CA->loc))
        {
            prevCloc = maybeP;
        ...
        // Step 2: N-C-O angle.
        axis = compute_normal(CA->loc, N->loc, HN->loc);
        theta = find_angle_along_vector(N->loc, prevOloc, prevCloc, axis);
        ...
        lv.copy(axis);
        lv.origin = N->loc;
        if (maybeP.get_3d_distance(HN->loc) > maybeM.get_3d_distance(HN->loc))
        {
            prevOloc = maybeP;
        }
        else
        ...
        axis = prevCloc.subtract(N->loc);
        theta = find_angle_along_vector(prevOloc, HN->loc, prevCloc, axis);
        ...
        axis = HN->loc.subtract(N->loc);
        theta = find_angle_along_vector(CA->loc, prevCloc, N->loc, axis);
        lv.copy(axis);
        lv.origin = N->loc;
...
void AminoAcid::predict_next_NHCA(Point* retval)
{
    ...
    for (i=0; i<10; i++)
    {
        ...
        // Rotate to get 120 degrees not clashing next.N with curr.CA.
        Point maybeP = rotate3D(nextNloc, C->loc, axis, plus);
        Point maybeM = rotate3D(nextNloc, C->loc, axis, minus);
        lv.copy(axis);
        lv.origin = C->loc;
        if (maybeP.get_3d_distance(CA->loc) > maybeM.get_3d_distance(CA->loc))
        {
        ...
        // Step 2: C-N-H angle.
        axis = compute_normal(CA->loc, C->loc, O->loc);
        theta = find_angle_along_vector(C->loc, nextHNloc, nextNloc, axis);
        plus  = triangular - theta;
        minus = triangular*2 - theta;
        ...
        lv.copy(axis);
        lv.origin = C->loc;
        if (maybeP.get_3d_distance(O->loc) > maybeM.get_3d_distance(O->loc))
        {
            nextHNloc = maybeP;
        }
        else
        ...
        // Step 3: Enforce trans configuration 180 degrees.
        axis = nextNloc.subtract(C->loc);
        theta = find_angle_along_vector(nextHNloc, O->loc, nextNloc, axis);
        ...
        // Rotate around C axis C-O to get 180 degrees about that axis.
        axis = O->loc.subtract(C->loc);
        theta = find_angle_along_vector(CA->loc, nextNloc, C->loc, axis);
        lv.copy(axis);
        lv.origin = C->loc;
...
void AminoAcid::attach_to_prediction(Point* predicted, bool CO)
{
    ...
    for (i=0; atoms[i]; i++)
    {
        ...
        Point pt = rotate3D(atoms[i]->loc, lv.origin, rot.v, rot.a);
        atoms[i]->move(pt);
    }
...
void AminoAcid::save_pdb(FILE* os, int atomno_offset)
{
    ...
    if (coordmtl && coordmtl->residue == residue_no)
    {
        coordmtl->save_pdb_line(os, i+1+atomno_offset);
    }
}
...
_return_added:
    ...
    {
        AABondDef* bb;
        for (j=0; bb = aadef->aabonds[j]; j++)
        {
            if (strcmp(bb->aname, atoms[i]->name) && strcmp(bb->bname, atoms[i]->name)) continue;
            if (bb->cardinality > 1.1 && bb->cardinality < 1.9) atoms[i]->aromatize();
        }
    }
...
float AminoAcid::distance_to_nearest_mclashables_atom()
{
    if (!mclashables) return 0;
    int i;
    Atom *a, *b;
    float result = Avogadro;
    for (i=0; mclashables[i]; i++)
    {
        if (mclashables[i] == this) throw 0xbadc0de;
        mutual_closest_atoms(mclashables[i], &a, &b);
        if (a && b)
        {
...
bool AminoAcid::can_reach(Atom* other) const
{
    ...
    r = ca1->loc.get_3d_distance(other->loc);
...
bool AminoAcid::can_reach(Molecule* other) const
{
    ...
    ca1 = get_atom("CA");
    ca2 = other->get_nearest_atom(ca1->loc);
    ...
    r = ca1->loc.get_3d_distance(ca2->loc);
...
bool AminoAcid::can_reach(AminoAcid* other) const
{
    ...
    if (cb1)
    {
        f = find_3d_angle(cb1->loc, ca2->loc, ca1->loc);
        f = 0.666 + 0.333 * cos(f);
    }
...
float AminoAcid::CB_angle(Point reference)
{
    ...
    if (!CB)
    {
        ...
        if (HA1 && HA2) CB = (HA1->loc.get_3d_distance(reference) < HA2->loc.get_3d_distance(reference)) ? HA1 : HA2;
        else if (HA1) CB = HA1;
        else if (HA2) CB = HA2;
        else return M_PI;
    }

    float result = find_3d_angle(CB->loc, reference, get_CA_location());
...
float AminoAcid::sc_hfe() const
{
    return aadef->sidechain_hydration_free_energy;
}
    ...
    for (m=0; m<2; m++)
    {
        for (i=0; nearby_mols[i]; i++)
        {
            ...
            for (j=0; atoms[j]; j++)
            {
                ...
                if (atoms[j]->Z == 1 && atoms[j]->is_polar() < 0.1) continue;

                if (fabs(atoms[j]->get_charge()) > 0.2) continue;

                Atom* a = nearby_mols[i]->get_nearest_atom(atoms[j]->loc);
                if (!a) continue;
                if (a->Z == 1) a = a->get_bond_by_idx(0)->atom2;
...
Atom* AminoAcid::capable_of_inter(intera_type inter)
{
    ...
    for (i=0; atoms[i]; i++)
    {
        ...
        {
            float r = atoms[i]->loc.get_3d_distance(caloc);
            if (r > cadist)
            {
                retval = atoms[i];
                cadist = r;
...
float AminoAcid::occlusion_by_mclashables()
{
    return Molecule::occlusion(mclashables);
}
...
void AminoAcid::hydrogenate(bool steric_only)
{
    ...
    if (aadef && get_atom("CB"))
    {
        for (i=0; aadef->aabonds[i]; i++)
        {
            ...
            Rotation rot = align_points_3d(at->get_atom("CB")->loc, get_atom("CB")->loc, get_CA_location());
            LocatedVector lv;
            lv = rot.v;
...
LocRotation AminoAcid::enforce_peptide_bond(bool cis)
{
    ...
    for (i=0; atoms[i]; i++)
    {
        Point ptnew = rotate3D(atoms[i]->loc, retval.origin, retval.v, retval.a);
        atoms[i]->move(ptnew);
...
LocRotation* AminoAcid::flatten()
{
    ...
    for (j=0; j<5; j++)
    {
        ...
        case 0:
            ...
            retval[0].v = v_from_pt_sub(localN->loc, prevC->loc);
            retval[0].origin = localN->loc;
            retval[0].a = 0;
            break;

        case 1:
            // Correction about N-HN axis.
            retval[1].v = v_from_pt_sub(localHN->loc, localN->loc);
            retval[1].origin = localN->loc;
            retval[1].a = 0;
            break;

        case 2:
            // Correction about C-O axis.
            retval[2].v = v_from_pt_sub(prevO->loc, prevC->loc);
            retval[2].origin = prevC->loc;
            retval[2].a = 0;
            break;

        case 3:
            // Phi correction.
            retval[3].v = v_from_pt_sub(localCA->loc, localN->loc);
            retval[3].origin = localCA->loc;
            retval[3].a = 0;
        ...
        case 4:
            // Psi correction.
            retval[4].v = v_from_pt_sub(localC->loc, localCA->loc);
            retval[4].origin = localCA->loc;
            retval[4].a = 0;
        ...
        for (i=0; i<250; i++)
        {
            ...
            if (j < 3)
            {
                lN = rotate3D(localN->loc, retval[j].origin, retval[j].v, retval[j].a+ad[j]);
                lCA = rotate3D(localCA->loc, retval[j].origin, retval[j].v, retval[j].a+ad[j]);
                lHN = rotate3D(localHN->loc, retval[j].origin, retval[j].v, retval[j].a+ad[j]);
                lC = rotate3D(localC->loc, retval[j].origin, retval[j].v, retval[j].a+ad[j]);
                lO = rotate3D(localO->loc, retval[j].origin, retval[j].v, retval[j].a+ad[j]);
            }
            else
            {
                lN = localN->loc;
...
float AminoAcid::estimated_stability(Molecule **neighbors)
{
    if (!aadef->flexion_probability) return 1;
    float grip = 0;

    int i, j;
    if (neighbors) for (i=0; neighbors[i]; i++)
    {
        if (neighbors[i] == this) continue;
        for (j=0; atoms[j]; j++)
        {
            if (atoms[j]->is_backbone) continue;
            Atom* a = neighbors[i]->get_nearest_atom(atoms[j]->loc);
            if (!a) continue;

            float r = a->distance_to(atoms[j]);
            if (r > _INTERA_R_CUTOFF) continue;
            r /= (a->vdW_radius + atoms[j]->vdW_radius);
            if (r < 1.0 - global_clash_allowance) continue;            // clashes are not stable.
            if (r < 1) r = 1;
    ...
    grip /= get_heavy_atom_count();

    return 1.0 - ((1.0 - pow(fmin(grip, 1), 2)) * fmin(1, aadef->flexion_probability * 3));
}
...
LocatedVector AminoAcid::rotate_backbone(bb_rot_dir direction, float angle)
{
    ...
    {
        atom2 = get_atom("N");
        if (atom2) atom2->move(rotate3D(atom2->loc, retval.origin, retval, angle));
    }
    ...
    if (direction == N_asc || direction == C_desc)
    {
        ...
        for (i=0; atoms[i]; i++)
        {
            if (!atoms[i]->is_backbone)
            {
                atoms[i]->move(rotate3D(atoms[i]->loc, retval.origin, retval, angle));
            }
        }
    }
...
Interaction AminoAcid::get_intermol_binding(AminoAcid** neighbs, bool backbone_atoms_only)
{
    ...
    else
    {
        ...
        for (i=0; neighbs[i]; i++)
        {
            ...
            {
                if (!atoms[j]->is_backbone) continue;
                Point aloc = atoms[j]->loc;
                for (k=0; k<neighbs[i]->atcount; k++)
                {
                    if (!neighbs[i]->atoms[k]->is_backbone) continue;
                    float r = neighbs[i]->atoms[k]->loc.get_3d_distance(&aloc);
...
bool AminoAcid::is_amide()
{
    ...
    for (i=0; atoms[i]; i++)
    {
        ...
        if (atoms[i]->get_family() == TETREL
            && atoms[i]->is_pi()
            && atoms[i]->is_bonded_to_pi(CHALCOGEN, true)
            && atoms[i]->is_bonded_to_pi(PNICTOGEN, true)
            )
            return true;
    }
