...
Point Point::add(Point* add_to) const
{
    Point retval;
    retval.x = x + add_to->x;
    retval.y = y + add_to->y;
...
Point Point::subtract(const Point subtracted) const
{
    Point retval;
...
Vector Vector::Fibonacci_sphere_vertex(int i, int num_rays)
{
    Vector v;
    v.theta = acos(1.0 - 2.0 * (0.5 + i) / num_rays);
    v.phi = M_PI * (1.0 + sqrt(5.0)) * (0.5 + i);
    v.r = 1;
    return v;
}
...
float cap_area(float r1, float r2, float d)
{
    if (d > r1+r2) return 0;
    return 2.0 * M_PI * r1 * r1 * ( 1.0 + (r2*r2 - r1*r1 - d*d) / (2.0*r1 * d) );
}
...
{
    if (pt.x < fmin(x1,x2) || pt.x > fmax(x1,x2)) return false;
    if (pt.y < fmin(y1,y2) || pt.y > fmax(y1,y2)) return false;
    if (pt.z < fmin(z1,z2) || pt.z > fmax(z1,z2)) return false;
    return true;
}
...
{
    Box result;
    result.x1 = fmin(x1, other.x1);
    result.y1 = fmin(y1, other.y1);
    result.z1 = fmin(z1, other.z1);
    result.x2 = fmax(x2, other.x2);
    result.y2 = fmax(y2, other.y2);
