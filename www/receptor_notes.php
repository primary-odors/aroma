...
function correlate_receptors_aromanotes()
{
    ...
    foreach (array_keys($prots) as $rcpid)
    {
        ...
        foreach ($odors as $oid => $odor)
        {
            ...
            {
                $adjustment = 1;
                if ($odor['full_name'] == 'indole' || $odor['full_name'] == 'skatole') $adjustment = 0.333;        // Two odorants that are dominating lists they oughtn't.
                if (isset($pair['adjusted_curve_top']))
                {
            ...
            while (strlen($btree) > 5)
            {
                $btree = substr($btree, 0, -1);
                if (isset($xvals[$btree][$oid])) $xvals[$btree][$oid] += $xval;
                else $xvals[$btree][$oid] = $xval;
            }
        }
    }
    ...
    foreach ($correlations as $btree => $pqcorrs)
    {
        if (isset($prots[$btree])) continue;
        $lbt = strlen($btree);
        foreach ($prots as $rcpid => $prot)
        {
            if (substr($prot['btree'], 0, $lbt) != $btree) continue;
            foreach ($pqcorrs as $pq => $corr)
            {
                if (@$correlations[$rcpid][$pq]) $correlations[$rcpid][$pq] += 0.2*$corr;
            }
        }
    }
