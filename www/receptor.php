...
if (isset($dock_results[$rcpid]))
{
    foreach ($dock_results[$rcpid] as $ligname => $dock)
    {
        ...
        $abn = $ibn = 1;
        $abe = $ibe = 9.16e+58;
        if (isset($dock["ascores"]))
        {
            foreach ($dock["ascores"] as $aln => $ale) if ($ale < $abe)
            {
                $abn = $aln;
                $abe = $ale;
            }
        }
        if (isset($dock["iscores"]))
        {
            foreach ($dock["iscores"] as $iln => $ile) if ($ile < $ibe)
            {
                $ibn = $iln;
                $ibe = $ile;
            }
        ...
        else if (isset($dock['a{$abn}_Pose1']) && isset($dock['i{$ibn}_Pose1']))
            $predictions[$oid] = (floatval($dock['i_Pose1']) - floatval($dock['a{$abn}_Pose1'])) / 2;
        $predab[$oid] = @floatval($dock['a{$abn}_Pose1']);
        $predib[$oid] = @floatval($dock['i{$ibn}_Pose1']);
        $predaff[$oid] = @floatval($dock['Affinity']);
        $preda6[$oid] = @floatval($dock['a{$abn}_BindingEnergy.6']);
    }
}

uasort($predictions, "predcmp");

$cmd = "ps -ef | grep -E ':[0-9][0-9] (bin/aromadock|bin/phew|obabel)' | grep -v grep";
$result = [];
exec($cmd, $result);
        ...
        window.setTimeout( function()
        {
            $('#viewer').on('load', function()
            {
                ...
                if (file_exists("../pdbs/$fam/$rcpid.active.pdb"))
                {
                    switch ($fam)
                    {
                        case "OR1": $colschm = "canaryScheme"; break;
                        case "OR2": $colschm = "amberScheme"; break;
                        case "OR3": $colschm = "greenScheme"; break;
                        case "OR4": $colschm = "purpleScheme"; break;
                        case "OR5": $colschm = "pinkScheme"; break;
                        case "OR6": $colschm = "cyanScheme"; break;
                        case "OR7": $colschm = "redScheme"; break;
                        case "OR8": $colschm = "orangeScheme"; break;
                        case "OR9": $colschm = "vermillionScheme"; break;
                        case "OR10": $colschm = "brownScheme"; break;
                        case "OR11": $colschm = "ashScheme"; break;
                        case "OR12": $colschm = "lemonScheme"; break;
                        case "OR13": $colschm = "tealScheme"; break;
                        case "OR14": $colschm = "blueScheme"; break;
                        case "OR51": $colschm = "beigeScheme"; break;
                        case "OR52": $colschm = "magentaScheme"; break;
                        case "OR56": $colschm = "emeraldScheme"; break;
                        case "TAAR": $colschm = "indigoScheme"; break;
                        default: 
                        $colschm = "skyScheme";
                    }
...
<div id="Info" class="tabcontent">
    ...
    ?>
    <h3>Estimated Expression: <?php echo @$receptor['expression'] ? ($receptor['expression']."% of the population.") : "Unknown."; ?></h3>
    <p>Expression estimate based on data from
    <a href="https://doi.org/10.1371/journal.pone.0096333">Verbeurgt et al (2014)</a>.
    </p>
...
<?php if (false && count($predictions)) { ?>
...
<a style="color: #00c;" href="https://github.com/primaryodors/aromadock">https://github.com/primaryodors/aromadock</a>.
</div>
<?php } ?>
...
foreach ($pairs as $oid => $pair)
{
    ...
    if (@$pair['ant_ref'])
    {
        $refno_ant = array_search($pair['ant_ref'], $lrefs);
        if (false === $refno_ant)
        {
            $refno_ant = count($lrefs)+1;
            $lrefs[$refno_ant] = $pair['ant_ref'];
        }
    }
    ...
    echo "<td>" . 
        ($disptop = ((@$pair['adjusted_curve_top'] || !@$pair['ec50'] || @$pair['type'])
    ...
    if (count($predictions))
    {
        ...
        {
            $lig = urlencode($predname[$oid]);
            echo "<td><a href=\"#\" onclick=\"show_dlmenu(event, '$rcpid', '$lig', '{$predate[$oid]}', '{$preddock[$oid]}', '{$predan[$oid]}', '{$predin[$oid]}');\">"
                .round($predictions[$oid], 4)."</a></td>";
            // echo "<td><a href=\"viewer.php?view=dock&prot=$rcpid&odor=$lig&mode=active\" target=\"_3d\">".round($predab[$oid], 4)."</a></td>";
            $pred_shown[$oid] = true;
        }
        else if (@$ligands_inprogress[$oid])
            echo "<td><span title=\"A prediction is currently running for this receptor+ligand pair.\">&#x23F3;</span></td>"; // <td>&nbsp;</td>";
        else echo "<td>&nbsp;</td>"; //<td>&nbsp;</td>";
    }

    $notes = implode(", ",make_clickable_notes($pq));
    foreach ($hilite as $h) $notes = str_replace(">$h<", "><span class=\"hilite\">$h</span><", $notes);
